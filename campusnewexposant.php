...
if (isset($_GET['t']))
{
 $action=$_GET['t'];
  ...
  switch($action)
  {
    ...
    case "article":
?>
<h2>AJOUTE UN NOUVEAU EXPOSANT</h2>
 

<form method="post" action="campusnewexposant.php" name="formulaire"  enctype="multipart/form-data">
...
<?php
	...
	echo '<i><font color="#ff0000"><small> Choisissez la photo de la baniere :</small></font></i><br />
...
//Vérification de COOLTAF :
    if (!empty($_FILES['expoimg']['size']))
    {
        ...
        {
                $cooltaf_erreur = "Erreur lors du transfert de la baniere : ";
        }
        ...
        {
                $i++;
                $cooltaf_erreur2 = "Baniere trop large ou trop longue : 
        ...
        {
                $i++;
                $cooltaf_erreur3 = "Extension de la baniere incorrecte";
        }
    }
?> 
   ...
   if ($i==0)
   {
     if (isset($_POST['message']))
     {
	  $vide="";
        ...
        $query=$db->prepare('INSERT INTO campus
        (groupe_name, temps)
        VALUES(:expocomment,:temps)');
     ...
     {
	  $baniereexpo=(!empty($_FILES['expoimg']['size']))?expo_photo($_FILES['expoimg']):''; 
	    $temps = time();
	 ...
	 $query=$db->prepare('UPDATE campus SET
        baniere=:baniereexpo WHERE id_campus=:lastid');
		$query->bindValue(':lastid', $lastid, PDO::PARAM_INT);
        $query->bindValue(':baniereexpo', $baniereexpo, PDO::PARAM_STR);  
        $query->execute();
        $query->CloseCursor(); 
		
		
		} else{ echo "La baniere  n'a pas ete envoye, Ficher vide";
