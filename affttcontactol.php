...
$TotalDesMembres=0;
$oui="oui";
$query=$db->prepare('SELECT membre_enligne, membre_id FROM forum_membres WHERE visibilite =:oui 
...
$lien='<a href="ttcontact.php?ol=IN&amp;p=';
...
$query=$db->prepare('SELECT  sexe, membre_avatar, membre_id,membre_pseudo, membre_enligne
        FROM forum_membres WHERE visibilite =:oui AND membre_enligne="oui" ORDER BY membre_pseudo ');
        $query->bindValue(':oui', $oui, PDO::PARAM_STR);
		$query->execute();
