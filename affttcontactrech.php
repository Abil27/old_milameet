...
$TotalDesMembres=0;
$oui="oui";
$query=$db->prepare('SELECT membre_pseudo, membre_id FROM forum_membres WHERE visibilite =:oui ORDER BY membre_pseudo');
$query->bindValue(':oui', $oui, PDO::PARAM_STR);
$query->execute();
 if (isset($_GET['chainerech']) ){
  //Début de la boucle
  
  while($data = $query->fetch())
  {
  if (strpos(stripslashes(htmlspecialchars("a".$data['membre_pseudo'])),stripslashes(htmlspecialchars( $_GET['chainerech'])))!=0){
...
$lien='<a href="ttcontact.php?chainerech='.$_GET['chainerech'].'&amp;p=';
$totalmsg=$TotalDesMembres; 
include("affnpagerech.php");
